...
# pkgs = c("igraph","RColorBrewer", "hexbin", "scales","grid", "lattice", "gdata", "gridExtra", "ape", "tcltk","reshape2", "ggplot2", "seqinr", "phangorn", "fs", "hash","ggdendro", "phytools","openxlsx","coop","tidyverse", "rstudioapi","reticulate") # package names
...
## run these 2 lines ONLY THE VERY FIRST TIME for installation of Python packages
...
setwd(data_dir)
#data_dir0 <- paste(path_home(),"/pomelo_outputs", sep="")
data_dir0 <- data_dir
...
## NOTE FILES ARE BEING SAVED IN SAVE DIRECTORY AS .RMD FILE, NOT THE data_dir
...
Sys.sleep(2)
PICthreshold <- readline(prompt = "Enter your desired PIC percentage thrshold (as an integer) : ")
...
CHECK TO SEE IF WE CAN USE PREVIOUSLY LOADED FILE FROM THE R CODE!!! - try using tk.choosefile alone before reading in tree or csv
...
```{python}
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Rickettsia2023-03-07.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Mycoplasma2023-03-08.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Treponema2023-03-15.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Mycobacterium2023-03-08.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Listeria2023-02-07.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Streptococcus2023-03-09.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Klebsiella2023-03-22.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Neisseria2023-03-09.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Haemophilus2023-03-23.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Campylobacter2023-03-09.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Chlamydia2023-03-30.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Yersinia2023-04-05.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Bartonella2023-03-16.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Francisella2023-04-05.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Burkholderia2023-03-07.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Yersinia2023-03-09.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Mycobacterium2023-02-10.nwk'))[0]
#t=glob.glob(os.path.join(base_path,'PATRICphylotree_midpointrooted_withlabels_Borrelia2023-03-22.nwk'))[0]
t=r.py_phylo00
...
for k in ll.getInternal(): ## iterate over internl nodes
...
Final large chunk that creates the figure. Note there is a place to change the PIC threshold that is shown in the figure. We recommend either 5.0 or 10.0 for 5% or 10% PIC change.
...
## identify the node from which listed tips descended
# ca=ll2.commonAncestor(ll2.getExternal(lambda w: '_'.join(w.name.split('_')[:-1]) in ['Wolbachia_endo_Ostrinia','Wolbachia_endo_Ctenocephalides']))
# # print(ca.leaves)
# ## collapse clade, give it a name and a width
# ll2.collapseSubtree(ca,'Wolbachia',widthFunction=lambda k: len(ca.leaves)/scale)

# ca=ll2.commonAncestor(ll2.getExternal(lambda w: '_'.join(w.name.split('_')[:-1]) in ['Rickettsia_monacensis','Rickettsia_parkeri']))
# # print(ca.leaves)
# ll2.collapseSubtree(ca,'Rickettsia 2',widthFunction=lambda k: len(ca.leaves)/scale)

# ca=ll2.commonAncestor(ll2.getExternal(lambda w: '_'.join(w.name.split('_')[:-1]) in ['Rickettsia_felis','Rickettsia_australis']))
# # print(ca.leaves)
# ll2.collapseSubtree(ca,'Rickettsia 1',widthFunction=lambda k: len(ca.leaves)/scale)

# ca=ll2.commonAncestor(ll2.getExternal(lambda w: '_'.join(w.name.split('_')[:-1]) in ['Magnetococcus_marinus','Brucella_suis']))
# # print(ca.leaves)
# ll2.collapseSubtree(ca,'outgroup',widthFunction=lambda k: len(ca.leaves)/scale)
...
########################## add delta PIC if it exceeds 20% - note change to 15%  abs(k.traits['delta_pic'])>=15.0. ######################## changing to py_PICthreshold
...
###### old manual way of saving
# path was plt.savefig('/Users/.../pathogen_metabolism_project/PIC_Streptococcus_Enterococcus250.pdf',dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Rickettsia20.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Campylobacter5.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Klebsiella5.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Haemophilus5.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Chlamydia10.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Bartonella10.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Francisella10.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Yersinia10.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Mycobacterium10.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Yersinia5.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Burkholderia95.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Bartonella1_Brucella50_10.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Mycoplasma10.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Neisseria5.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_TreponemaBorellia95.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Borellia10.pdf'),dpi=300,bbox_inches='tight')
#plt.savefig(os.path.join(base_path,'PIC_Streptococcus_Enterococcus_test.pdf'),dpi=300,bbox_inches='tight')
