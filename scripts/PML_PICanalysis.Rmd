...
setwd("~/code")
data_dir <- "~/code"
data_dir0 <- paste(path_home(),"/code", sep="")
...
PICplot_titlepdf <- paste("PIC_",plot_title, ".pdf", sep="")
...
########################## add delta PIC if it exceeds 20% - note change to 5%  abs(k.traits['delta_pic'])>=5.0. ########################
ll2.addText(ax,x_attr=lambda k: k.height-k.length/2,y_attr=lambda k: k.y+0.15,target=lambda k: 'delta_pic' in k.traits and abs(k.traits['delta_pic'])>=15.0,text=lambda k: '+%.1f%%'%(k.traits['delta_pic']) if k.traits['delta_pic']>0 else '%.1f%%'%(k.traits['delta_pic']),path_effects=effects,size=10,va='bottom',ha='center')
